...
class Abin {
  private:
    ...
    //AÑADIDO (PRIVADO)
    static void consulta(node *now, T elem, bool &trobat);
...
Abin<int> crear_arbre(){	//se crea por preordre
	Abin<int> T;
	int n;
	cin >> n;
	if(n != -1){	//mientras no sea el final
		Abin<int> f_izq = crear_arbre();	//crear hijo izq
		Abin<int> f_der = crear_arbre();	//crear hijo der
		T = Abin<int>(f_izq, n ,f_der);		//crear subarbre 
	}
	return T;
}

template <typename T>
bool Abin<T>::consulta(T elem) const{
	bool trobat = false;
	consulta(_arrel, elem, trobat);
	return trobat;
}

template <typename T>
void Abin<T>::consulta(node *now, T elem, bool &trobat){
	
	if( now != NULL ){
		if(now->info == elem) trobat = true;
		else if(now->info > elem){	//si elem es menos que el arrel, se guarda en el fill esq
		
			//hacemos la llamada recursiva para insertar en el lugar que le toca el elem en el subarbre esq
			consulta(now->f_esq, elem, trobat);	
		}
			...
			//hacemos la llamada recursiva para insertar en el lugar que le toca el elem en el subarbre dret
			 consulta(now->f_dret, elem, trobat);	
		}
...
int main(){
	int mida;
	cin >> mida;	//ignoramos la mida
	
	Abin<int> B = crear_arbre();
	
	int n;	//Enunicado : se insertan numeros naturales
	
	while(cin >> n){
		
		bool trobat = B.consulta(n);	//consultar si existe el elemento en el arbol
		if (trobat) cout << n << " 1" << endl;
		else cout << n << " 0" << endl;
	}
