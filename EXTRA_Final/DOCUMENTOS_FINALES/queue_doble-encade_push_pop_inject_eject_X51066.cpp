...
// Post: Insereix un element al DAVANT de la deque.
	...
	node* p = new node;
	p->info = e;
	if (_prim == NULL){//no hi ha elements
		p->seg = NULL;
		_ult = p;
	}
	else{//hi ha mes d'un element
		p->seg = _prim;
		_prim->ant = p;
	}
	p->ant = NULL;

	_prim = p; //primer elem pq s'inserta davant
	_long++;
}
...
// Pre: True
// Post: Insereix un element al DETRAS de la deque.

	node *p = new node;
	p->info = e;
	if (_prim == NULL){//cap element a la cua
		 p->ant = NULL;
		 _prim = p;
	}
	else{//hi ha elem.
...
// Post: Elimina el primer element de la deque.
	...
	if (_prim != NULL){
	 node* p = _prim; //node aux
	 
	 if (_long > 1){//mes d'un elem
	 	_prim = _prim->seg;
	   	_prim->ant = NULL;
	 }
	 else{//nomes 1 elem, fer node buit
	   	_prim = NULL;
	   	_ult = NULL;
	 }
	 delete p;
