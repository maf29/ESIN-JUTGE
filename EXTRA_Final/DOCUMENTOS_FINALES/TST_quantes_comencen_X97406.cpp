...
// Post: Retorna el nº de claus que comencen per algun dels caràcters que conté inicials
	...
	int num = 0;
	if (_arrel != NULL){//arbre no buit
		int i = 0;
		node* p;
		//recorre llista de simbols
		while (i < inicials.size()){
		 	p = prefix(inicials[i], _arrel);
		 	//s'ha trobat el prefix, contar el num 				  de words
...
dicc::node* dicc::prefix(char c, node *p){
	...
	if (p != NULL){
	//per fer-ho el metode "insereix" es util
		//símbol introduit menor--> anar x esq
		if (p->_c > c) n = prefix(c, p->_esq);
		//símbol introduit major--> anar x dreta 
		else if (p->_c < c) n = prefix(c, p->_dre);
		//simbol igual--> sha trobat
		else if (p->_c == c) n = p; 
	}
	return n;
}
...
nat dicc::nombre(node *p){
	...
	char fin = '#';
	if (p != NULL){
		//conta el num de paraules que es compleixen
		num = nombre(p->_esq) + nombre(p->_cen) + nombre(p->_dre);
		if (p->_c == fin) num++; //final de paraula
	}
	return num;
}
