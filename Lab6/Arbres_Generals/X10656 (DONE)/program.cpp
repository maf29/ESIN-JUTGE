 ...
 private:
   ...
   static void quants_grau(nat n, node *now, nat &cont);
 };
...
nat Arbre<T>::quants_grau(nat n) const{
	nat cont = 0;
	quants_grau(n, _arrel, cont);
	return cont;
}
template <typename T>
void Arbre<T>::quants_grau(nat n, node *now, nat &cont){
	if(now != NULL){
		if(now->primf != NULL) quants_grau(n, now->primf, cont);
		if(now->seggerm != NULL) quants_grau(n, now->seggerm, cont);
