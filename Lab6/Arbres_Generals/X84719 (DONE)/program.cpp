#include <algorithm>
 ...
 private:
   ...
   static void grau_max(nat &max, node *now);
 };
 
 // Aquí va la implementació del mètode grau_max
 // Retorna grau màxim dels nodes de l’arbre
template <typename T>
...
template <typename T>
void Arbre<T>::grau_max(nat &max, node *now){
	if(now != NULL){
		if(now->primf != NULL){
			grau_max(max, now->primf);
		}
		if(now->seggerm != NULL){
			grau_max(max, now->seggerm);
		}
		if(now->primf != NULL){
			node *aux = now->primf;
			nat cont = 0;
